...
def _build_raft_bases():
    """Build the base classes for RAFT, optionally including HuggingFace Hub mixin."""
    bases = [nn.Module]
    if _HUB_MIXIN_AVAILABLE:
        bases.append(PyTorchModelHubMixin)
    return tuple(bases)


class RAFT(*_build_raft_bases()):
