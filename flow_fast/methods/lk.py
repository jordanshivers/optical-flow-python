"""
Lucas-Kanade optical flow estimation (accelerated version).

B.D. Lucas and T. Kanade. "An iterative image registration technique
with an application to stereo vision." IJCAI, 1981.

Uses cv2.filter2D for Gaussian-weighted structure tensor computation.
"""
import copy
...
class LKOpticalFlow(BaseOpticalFlow):
    ...
    This is the accelerated version using OpenCV for filtering.
    """
    ...
    def compute_flow_base(self, uv):
        ...
        G = fspecial_gaussian(self.window_size, sigma).astype(np.float64)
        ...
        for i in range(self.max_warping_iters):
            ...
            # Structure tensor components using cv2.filter2D
            Sxx = cv2.filter2D(Ix * Ix, -1, G, borderType=cv2.BORDER_REFLECT)
            Sxy = cv2.filter2D(Ix * Iy, -1, G, borderType=cv2.BORDER_REFLECT)
            Syy = cv2.filter2D(Iy * Iy, -1, G, borderType=cv2.BORDER_REFLECT)
            Sxt = cv2.filter2D(Ix * It, -1, G, borderType=cv2.BORDER_REFLECT)
            Syt = cv2.filter2D(Iy * It, -1, G, borderType=cv2.BORDER_REFLECT)
